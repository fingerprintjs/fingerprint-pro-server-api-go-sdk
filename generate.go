...
// fixIntegrationDocs fixes a wrong link to the IntegrationSubintegration.md
func fixIntegrationDocs() {
	path := "docs/Integration.md"

	// Check if the documentation file exists
	if _, err := os.Stat(path); os.IsNotExist(err) {
		log.Printf("Warning: %s not found, skipping fix", path)
		return
	}
	...
	newContents := strings.ReplaceAll(string(contents), "Integration_subintegration", "IntegrationSubintegration")

	err = os.WriteFile(path, []byte(newContents), 0644)
	if err != nil {
		log.Fatal(err)
	}
	log.Println("Fixed links in docs/Integration.md")
}
